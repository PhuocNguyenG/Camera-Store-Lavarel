                        ...
                        <div class="panel-body">
                            ...
                            <div class="position-center">
                                ...
                                <div class="form-group">
                                    ...
                                    <img src="{{URL('public/uploads/products/'.$pro->product_image)}}"height="70px" witdh="70px">
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputDesc">Mô tả sản phẩm</label>
                                    <textarea style = "resize: none" rows="6" class="form-control" name= "product_desc" id="ckeditor4"
                                    id="exampleInputPassword1">{{$pro->product_desc}}</textarea>
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputDesc">Nội dung sản phẩm</label>
                                    <textarea style = "resize: none" rows="6" class="form-control" name= "product_content" id="ckeditor5"
                                ...
                                <div class="form-group">
                                    <label for="exampleInputStatus">thương hiệu</label>   
                                    <select name ="product_brand" class="form-control input-sm m-bot15">
                                        @foreach($brand_product as $key => $brand)
                                        @if($brand->brand_id == $pro->brand_id)
                                            <option selected option value = "{{$brand->brand_id}}">{{$brand->brand_name}}</option>
                                        @else
                                            <option option value = "{{$brand->brand_id}}">{{$brand->brand_name}}</option>
                                        @endif
                                        @endforeach
                            </select>
                                </div>
